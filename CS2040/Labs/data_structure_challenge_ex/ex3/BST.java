...
class BSTVertex {
    BSTVertex(int v) { key = v; parent = left = right = null; height = 0; size = 1; }
...
public class BST {
    ...
    private void resize(BSTVertex T) {
        ...
        T.height = Math.max(leftH, rightH) + 1;
        T.size = leftS + rightS + 1;
    }

    private BSTVertex leftRotate(BSTVertex T) {
        BSTVertex subRoot = T.right;
        subRoot.parent = T.parent;
        T.parent = subRoot;
        T.right = T.right.left;
        if (T.right != null) T.right.parent = T;
        subRoot.left = T;
        resize(subRoot.left);
        if (subRoot.right != null) resize(subRoot.right);
        resize(subRoot);
        return subRoot;
    }

    private BSTVertex rightRotate(BSTVertex T) {
        BSTVertex subRoot = T.left;
        subRoot.parent = T.parent;
        T.parent = subRoot;
        T.left = T.left.right;
        if (T.left != null) T.left.parent = T;
        subRoot.right = T;
        resize(subRoot.right);
        if (subRoot.left != null) resize(subRoot.left);
        resize(subRoot);
        return subRoot;
    }

    // public method to get rank of vertex containing key with value v from BSt
    public int getRank(int v) { return rank(root, v); }

    // helper recursive method to find rank
    private int rank(BSTVertex T, int v) {
        int TLeftSize;
        if (T.left == null) TLeftSize = 0; 
        else TLeftSize = T.left.size;

        if (T.key == v) return TLeftSize + 1;
        else if (T.key > v) return rank(T.left, v);
        else return TLeftSize + 1 + rank(T.right, v);
    }
